...
# Define constants
PATH = "media/Notes/E0.wav"     # Path to .wav file from project root
BIN_SIZE = 1.00                 # Size of bin in seconds
...
# Extract the n highest frequencies at each time step
def extract_peak_series(fft_series, n):

    # Get the sorted n loudest frequencies at the time step
    def extract_peak_indices(arr):
        idx = np.argpartition(arr, -n)[-n:]
        return idx[np.argsort(-arr[idx])]

    # Apply the extract_peak_indices function to each time step
...
peak_series = extract_peak_series(fft_series, 3)
animate_results(fft_series, peak_series)
